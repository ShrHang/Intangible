...
当玩家处于无实体状态时，可以像幽灵一样飞行、穿过墙体，从而更方便地进入大型建筑、自动化机器和复杂结构的内部进行检查与修改。
...
它们的持续时间、配方输入药水和配方材料都可以在`intangible-startup.toml`中调整（在 1.20.1 forge 中是`intangible-startup.config`）。
