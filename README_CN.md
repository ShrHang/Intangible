...
当你终于搭好了一个巨大的工厂、一套复杂的管线系统，或者一台层层嵌套的多方块机器，却发现某个需要调整的方块藏在结构最深处，  
这时你或许就会希望自己能短暂地像幽灵一样行动，钻进建筑、流水线和机器的缝隙里，改掉那个小小的问题就出来，不破坏任何无关结构。

**Intangible** 正是为这种场景而生。

这个名称灵感来自《杀戮尖塔》的模组添加了一个简单但强大的状态效果：**无实体**。

当玩家处于无实体状态时，可以像幽灵一样飞行、穿过墙体，从而更方便地进入大型建筑、机器等复杂结构的内部进行检查与修改。
...
它们的持续时间、配方输入药水和配方材料都可以在`intangible-startup.toml`中调整。（在1.20.1中是`intangible-startup.config`）

这些配置属于启动配置，修改后需要重启游戏或服务端才能稳定生效。

## 渲染

如果你不想显示无实体玩家的半透明覆盖层或者想修改颜色及其透明度，可以在`intangible-client.toml`中关闭。

如果你想修改覆盖层的纹理，可以在资源包中添加`assets/intangible/textures/entity/intangible_overlay.png`文件。